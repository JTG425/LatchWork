...
- **Memory circuits** — the *Memory* palette folder includes JK Flip-Flop, SR Latch, D Latch, D Flip-Flop, SR Flip-Flop, T Flip-Flop, and Shift Register primitives. The JK/T parts default to falling-edge operation so `Q → CLK` ripple counters count cleanly.
...
Before shipping a change, run the automated engine/VHDL regressions, the
